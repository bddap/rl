...
//!
//! The determinism of [`crate::net::sim`] + [`crate::net::lockstep`] is sacrosanct,
//! and the deployed peers pair over one specific lockstep ALPN
//! ([`crate::net::transport::ALPN`]). So telemetry is built to be provably unable to
//! touch either:
//!
...
use crate::net::lockstep::Fault;
use crate::net::sim::{Input, Outcome, Sim};
...
/// [`crate::net::transport::ALPN`] (the lockstep wire) on purpose: a telemetry dial
...
/// [`SERVICE_NAME`](crate::net::transport::SERVICE_NAME): if telemetry reused the game's
