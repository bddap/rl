...
//! player is the bottleneck, not animations). Since the post-playtest pick
...
fn spawn_chord_map(mut commands: Commands, mut images: ResMut<Assets<Image>>) {
 ...
 // Full-screen (the amendment): a dark backdrop over the whole
...
fn drive_chord_map(
    ...
    if !chords.capturing() {
 // rl#381 directive: the map is GONE the frame the X hold ends — no
