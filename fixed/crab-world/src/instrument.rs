...
//!
//! The press→sound mapping is DATA, not code (amendment on rl#359): a scheme is
...
//!
//! THE scheme is **heldbreath** on **A hirajoshi** — the pick from the design
//! exploration (rl#359, 2026-08-12: "held breath hirajoshi"). Entering a
...
/// A hirajoshi on A2 (A B C E F) — the by-ear pick on the web playground.
...
pub struct NoteSpec {
 ...
 /// 0 = clean; the deep-code tension layer (spec on rl#380).
    pub crush: f32,
    pub gain: f32,
}
...
/// The deep-code tension curves (rl#380 spec), by note index
...
/// too intense in play (directive: "Reduce max to 1/3 of current") — the
...
/// Completion = cadence. Accepted is the EXHALE, and per the amendment
...
impl NoteVoice {
 ...
 // specified pop-in) holds every 3rd sample (~15 kHz) at ~10 bits —
...
mod tests {
 ...
 /// The accepted chord is DERIVED from the code's own melody (amendment
