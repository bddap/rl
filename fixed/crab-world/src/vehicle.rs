...
// rl#379 (directive: "2x current"): every equilibrium speed doubled by scaling the
...
const PLANE: VehicleParams = VehicleParams {
 ...
 // 1.8 the plane out-lifted its ~2.6 N weight from 1.4 m/s (directive: "Bernoulli overdone",
...
// rl#307 feel spec (directive): rotational inertia — a released spin persists and is bled
...
mod tests {
 ...
 // rl#379: the set target is 2× the pre-retune measurement (4.635 m/s,
 ...
 /// rl#316 regression (directive: "ship shouldn't only sink when holding still — gravity
 ...
 /// rl#313 (directive): the idle sink must FEEL like gravity — an integrated force
