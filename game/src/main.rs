...
enum Command {
    ...
    NnCrabVehicleStability(NnCrabVehicleStabilityArgs),
}
...
fn main() -> Result<()> {
    ...
    match command {
        ...
        Command::NnCrabVehicleStability(args) => run_nn_crab_vehicle_stability(args),
    }
}
...
#[derive(Parser)]
struct NnCrabVehicleStabilityArgs {
    /// Trained crab checkpoint dir (`brain.bin` + `normalizer.bin`). Same resolution as
...
/// Crab-policy-stability gate (see [`Command::NnCrabVehicleStability`]): run the trained crab, ram
/// it with a real vehicle rigidbody, and confirm the trained walking survives — the migration's
/// DONE bar #2. Three checks, all headless (physics + inference, no GPU): the carapace stays FINITE
/// throughout (no NaN/explosion), it never LAUNCHES out of the arena (bounded height), and it
/// RECOVERS its standing posture + reach after the hit. Exits nonzero on any failure.
fn run_nn_crab_vehicle_stability(args: NnCrabVehicleStabilityArgs) -> Result<()> {
    ...
    for s in result.samples.iter().filter(|s| {
        s.tick.is_multiple_of(50) || (s.tick >= ram_tick && s.tick < ram_tick + 20)
    }) {
