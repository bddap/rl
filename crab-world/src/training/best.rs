...
impl BestKeeper {
    ...
    pub(crate) fn maybe_snapshot(&mut self) -> bool {
        ...
        // The far per-bearing vector rides every candidate log line (rl#276): the min
        // headline collapses a one-bearing hole into an ambiguous scalar — the vector
        // names the failing bearing the moment it opens. Never feeds the promotion
        // decision.
        let far = format!("far bearings {}", report.progress_line());
        if !beats {
            info!(
                "[best] chase-eval: min-bearing progress {:.3} m (reached={}) vs bar {bar} — keeping incumbent | {far}",
                candidate.get(),
                report.reached()
            );
            return true;
