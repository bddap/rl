...
/// The per-horizon step telemetry a rollout worker accumulates and drains wholesale
/// at `end_horizon` — one struct, one `mem::take`, so a new counter cannot forget its
/// drain or its slot in [`HorizonOutput`].
#[derive(Default, Clone, Debug)]
pub(crate) struct StepTelemetry {
    /// Carapace planar drift-from-spawn as `(sum, count)` over recording-env ticks.
    pub(crate) drift_sum: f64,
    pub(crate) drift_count: u64,

    /// Per-episode reach tally: episodes that touched the target / episodes finished.
    pub(crate) reach_reached: u64,
    pub(crate) reach_finished: u64,
    ...
    pub(crate) reach_by_bearing: [(u64, u64); crate::eval::EVAL_BEARINGS],

    pub(crate) progress_glitch_drops: u64,

    pub(crate) nonfinite_obs_elements: u64,

    pub(crate) hard_ccd_link_clamp_candidates: u64,
    pub(crate) hard_ccd_link_clamp_candidates_max: u64,
}

impl StepTelemetry {
    /// Pool another horizon's counters into this one (the learner's cross-thread merge).
    pub(crate) fn absorb(&mut self, other: &StepTelemetry) {
        self.drift_sum += other.drift_sum;
        self.drift_count += other.drift_count;
        self.reach_reached += other.reach_reached;
        self.reach_finished += other.reach_finished;
        for (m, o) in self.reach_by_bearing.iter_mut().zip(other.reach_by_bearing) {
            m.0 += o.0;
            m.1 += o.1;
        }
        self.progress_glitch_drops += other.progress_glitch_drops;
        self.nonfinite_obs_elements += other.nonfinite_obs_elements;
        self.hard_ccd_link_clamp_candidates += other.hard_ccd_link_clamp_candidates;
        self.hard_ccd_link_clamp_candidates_max = self
            .hard_ccd_link_clamp_candidates_max
            .max(other.hard_ccd_link_clamp_candidates_max);
    }
}

/// Rollout-worker-only state: the live envs and everything the stepping systems
/// mutate per tick. A learner never steps envs, so none of this exists on it.
pub(crate) struct WorkerMode {
    pub(super) envs: Vec<EnvEpisode>,
...
/// the resume key. A worker never saves or updates, so none of this exists on it.
pub(crate) struct LearnerMode {
    config: PpoConfig,
...
/// Training state, split by mode at the type level (bddap/rl#337): `mode` holds what
/// exists ONLY on that side, so "worker with a resume key" or "learner with live
/// envs" doesn't typecheck. The flat fields are genuinely shared by both.
pub(crate) struct TrainingState<M> {
...
pub(crate) type WorkerState = TrainingState<WorkerMode>;
pub(crate) type LearnerState = TrainingState<LearnerMode>;
...
fn seeded_base(
    ...
    // flag↔tag check.
    let brain: AnyBrain<TrainBackend> =
...
impl LearnerState {
    ...
    /// The PPO update's knobs (the σ-anneal schedule lives here too).
    pub(crate) fn ppo_config(&self) -> &PpoConfig {
        &self.mode.config
    }
...
/// The learner's warm-start from the checkpoint's envelope tag — the tag is
/// AUTHORITATIVE for which architecture resumes (bddap/rl#200 §2/§3). The refusal
/// policy here is ABORT, not cold-start: a present-but-unusable brain (legacy,
/// corrupt, unknown arch/version, mis-copied) must never silently discard the trained
/// policy of a live run by cold-starting over it. The ONE deliberate cold start is a
/// DOF change (bddap/rl#31): same arch, but the checkpoint's obs/action widths — read
...
///
/// LEARNER-ONLY (now by type, bddap/rl#337): a rollout worker's brain + obs normalizer
/// are replaced by the learner's snapshot at the first `begin_horizon` (a failed load
/// there refuses the horizon), and the return normalizer / resume key are learner-side
/// state — so a worker read would be K redundant disk loads, where this abort policy
/// would misfire on a fresh dir.
///
/// Returns `Some(key)` — the resumed set's pairing key (bddap/rl#200 §2 + #215),
/// which every paired artifact must match — once the brain warm-starts. `None` on
/// every cold start.
fn warm_start(
    config: &TrainConfig,
    requested: Option<ArchId>,
    device: &NdArrayDevice,
    brain: &mut AnyBrain<TrainBackend>,
    obs_normalizer: &mut ObsNormalizer,
    return_normalizer: &mut ReturnNormalizer,
) -> Option<SetKey> {
        ...
        Ok(file) => {
            ...
            if crate::policy::dims_fit_rig(obs, action) {
                ...
                *brain = loaded;
                ...
                // still authoritative — without a flag the default could otherwise
                // silently switch a non-default run's architecture). Guarded so the
                // matching-arch case keeps the FIRST init (a second init consumes
                // another backend-RNG draw, which would change a fixed-`--seed` cold
                // start's initial weights for nothing).
...
impl WorkerState {
    ...
    pub(crate) fn begin_horizon(&mut self, req: HorizonRequest) -> bool {
        ...
        let normalizer_ok = self.obs_normalizer.load_snapshot(req.normalizer);
        ...
        self.mode.log_std_floor = req.log_std_floor;
        self.mode.increment = IncrementAccumulator::new();
        true
    }
    ...
    pub(crate) fn end_horizon(&mut self) -> HorizonOutput {
        HorizonOutput {
            ...
            increment: self.mode.increment.increment(),
            rewards: self.drain_finished_episode_rewards(),
            telemetry: std::mem::take(&mut self.mode.telemetry),
        }
    }
    ...
    fn take_rollouts(&mut self) -> Vec<RolloutBuffer> {
        (0..self.mode.rollouts.len())
            .map(|e| RolloutBuffer {
                transitions: std::mem::take(&mut self.mode.rollouts[e].transitions),
                bootstrap: self.mode.envs[e].pending.as_ref().map(|p| p.value),
            })
            .collect()
    }

    /// Physics ticks stepped since this worker was built (the horizon odometer).
    pub(crate) fn total_steps(&self) -> u64 {
        self.mode.total_steps
    }

    fn drain_finished_episode_rewards(&mut self) -> Vec<f32> {
        let out = self.recent_rewards[self.mode.reported_episodes..].to_vec();
        self.mode.reported_episodes = self.recent_rewards.len();
        out
    }
}

impl<M> TrainingState<M> {
...
mod tests {
    ...
    fn inference_cache_is_bit_identical_and_refreshes_on_weight_change() {
        ...
        // any learner/worker constructor) can make two draws straddling its reseed come back
    ...
    fn brain_init_is_reproducible_from_the_backend_seed() {
        ...
        let init_with_seed = |seed: u64| -> Vec<u32> {
            <TrainBackend as burn::tensor::backend::Backend>::seed(&device, seed);
            policy_bits(&AnyBrain::<TrainBackend>::init(ArchId::DEFAULT, &device).valid())
        };
