...
// ─── THIS LOOK: VIOLET NOCTURNE (wet-nocturne variant, twilight-violet grade) ─
// Same wet-ground structure as wet_nocturne — moisture field, mirror puddles,
// dew glints — pushed toward deep twilight violet: dry dust lifts lavender-grey,
// wet earth sinks to indigo, puddles hold violet-black, and the dew field is
// DENSER (~2× the glints) in icy lilac. Static palette shift only; every mask
// and octave is byte-identical to wet_nocturne.
...
fn fragment(
    ...
    // Nocturne grade: deep twilight violet.
    rgb = mix(rgb, rgb * vec3(1.00, 0.84, 1.22), 0.60);
    ...
    rgb *= mix(vec3(1.0), vec3(1.14, 1.10, 1.18), (1.0 - moist) * 0.45 * S.x);
    rgb *= mix(vec3(1.0), vec3(0.52, 0.50, 0.74), moist * 0.90 * S.x);
    ...
    rgb *= 1.0 - 0.45 * rim;
    // Puddle body: violet-black with a whisper of dusk.
    rgb = mix(rgb, vec3(0.065, 0.05, 0.11), pud * 0.92);
    ...
    if spark_f > 0.001 {
        ...
        // Denser dew than wet_nocturne (0.97 vs 0.985 — ~2× the glints): the
        // violet grade reads darker overall, so the dew field carries more of
        // the near-field sparkle duty.
        if h > 0.97 {
            ...
            let amt = spark_f * star * ndv * (h - 0.97) / 0.03 * (0.35 + 0.65 * snow);
            out.color = vec4(out.color.rgb + vec3(0.82, 0.72, 1.00) * amt, out.color.a);
        }
    }
