...
// ─── THIS LOOK: TWINKLE NOCTURNE (wet-nocturne variant, ANIMATED) ────────────
// Same wet-ground structure as wet_nocturne, ALIVE at the small scale: every
// dew glint twinkles on its own phase and rate (per-cell hash → sinusoid on
// globals.time, so the field glitters instead of sitting frozen), the dew is
// denser to sell it, and the puddle surfaces carry a slow drifting moon-sheen
// shimmer — water that reads as barely disturbed, not glass. All animation is
// post-lighting (glints and sheen are light, not albedo); masks and octaves
// are byte-identical to wet_nocturne.
...
fn fragment(
    ...
    // Puddle shimmer (ANIMATED): a slow drifting brightness swell over the
    // water — the moon sheen wandering on a barely-disturbed surface. Two
    // counter-drifting meter-scale noise fields, footprint-faded so distant
    // water calms to stillness instead of fizzing.
    if pud > 0.001 {
        let shim = vnoise(p / 3.5 + vec2(t * 0.16, -t * 0.11), 214u)
            + 0.5 * vnoise(p / 1.1 + vec2(-t * 0.23, t * 0.19), 215u) * footprint_fade(1.1, fw);
        out.color = vec4(
            out.color.rgb * (1.0 + 0.30 * pud * shim * footprint_fade(3.5, fw)),
            out.color.a,
        );
    }
