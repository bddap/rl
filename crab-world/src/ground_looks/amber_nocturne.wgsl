...
// ─── THIS LOOK: AMBER NOCTURNE (wet-nocturne variant, harvest-moon grade) ────
// Same wet-ground structure as wet_nocturne — moisture field, mirror puddles,
// dew glints — regraded under a low HARVEST moon: the cool blue cast becomes
// warm amber, dry dust lifts toward pale gold, wet earth sinks to umber, and
// puddles hold a whisper of ember instead of night sky. Static palette shift
// only; every mask and octave is byte-identical to wet_nocturne.
...
fn fragment(
    ...
    // Nocturne grade: warm everything — a harvest moon, not a cold one.
    rgb = mix(rgb, rgb * vec3(1.18, 1.00, 0.78), 0.55);
    ...
    rgb *= mix(vec3(1.0), vec3(1.22, 1.12, 0.92), (1.0 - moist) * 0.45 * S.x);
    rgb *= mix(vec3(1.0), vec3(0.66, 0.57, 0.46), moist * 0.90 * S.x);
    ...
    rgb *= 1.0 - 0.45 * rim;
    // Puddle body: deep umber-black with a whisper of ember.
    rgb = mix(rgb, vec3(0.085, 0.06, 0.038), pud * 0.92);
    ...
    if spark_f > 0.001 {
        ...
        if h > 0.985 {
            ...
            out.color = vec4(out.color.rgb + vec3(0.95, 0.78, 0.48) * amt, out.color.a);
        }
    }
