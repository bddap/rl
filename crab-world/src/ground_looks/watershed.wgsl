...
//                                                          patterned_ground
//
// Moisture is the spine, and it is TOPOGRAPHIC — water runs downhill and pools
// on the flat — so the wet/dry map a player reads from the plane is the same map
// the terrain silhouette already told them. That is what makes six languages read
// as one place instead of six wallpapers: nothing is decorative, every language is
// the visible consequence of where water is.
//
...
fn fragment(
    ...
    // MOISTURE — the spine. A domain-warped fbm biased by topography: high
    // ground and steep ground shed water, low flats hold it. Field alone would
    // paint wetness anywhere; the topographic terms are what make the wet map
    // agree with the silhouette a player already reads from the plane.
    let mwarp = vec2(vnoise(p / 300.0, 111u), vnoise((p + vec2(5.1, 1.7)) / 300.0, 112u)) * 90.0;
    let pm = p + mwarp;
    let m_fbm = vnoise(pm / 520.0, 113u) * 0.35
        + vnoise(pm / 150.0, 114u) * 0.30
        + vnoise(pm / 48.0, 115u) * 0.35;
    let shed = 0.42 * smoothstep(-1200.0, 300.0, wp.y) + 0.48 * smoothstep(0.10, 0.48, steep);
    let moist = smoothstep(0.00, 0.32, m_fbm + 0.22 - shed) * (1.0 - snow);
    ...
    if wet_w > 0.003 {
        ...
        // mid-altitude frame. Fine noise frays the shoreline up close.
        ...
        pud = clamp(pud_core * flat * S.x * wet_w, 0.0, 1.0);
        // Soaked rim just outside each pool — darker than either neighbor.
        let rim = clamp(smoothstep(0.32, 0.46, pud_field) - pud_core, 0.0, 1.0) * flat * wet_w;
        rgb *= 1.0 - 0.45 * rim;
