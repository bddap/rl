...
// ─── THIS LOOK: AURORA NOCTURNE (wet-nocturne variant, ANIMATED) ─────────────
// Same wet-ground structure as wet_nocturne, with an ANIMATED aurora washing
// over it: kilometer-scale green→violet curtains drift slowly across the world
// (globals.time), glowing faintly on wet ground and strongly in the mirror
// puddles — the water "catches" the sky. The glow is added POST-lighting (it is
// light arriving, not albedo) and drifts at aurora pace: a full curtain takes
// ~a minute to cross a valley. Masks and octaves are byte-identical to
// wet_nocturne; only the post-lighting stage differs.
...
fn fragment(
    ...
    // Aurora wash (ANIMATED): two drifting curtain fields, green and violet,
    // kilometer wavelengths so the bands read from the plane and sweep visibly
    // on foot. Sub-pixel safety is inherent (km wavelengths never alias); the
    // glow scales with wetness — dry dust barely catches it, puddles mirror it.
    let t = globals.time;
    let cur1 = vnoise(vec2(p.x * 0.0011 + t * 0.040, p.y * 0.00045 - t * 0.016), 210u)
        + 0.5 * vnoise(vec2(p.x * 0.0032 - t * 0.028, p.y * 0.0013 + t * 0.012), 211u);
    let cur2 = vnoise(vec2(p.x * 0.0008 - t * 0.024, p.y * 0.0006 + t * 0.033), 212u)
        + 0.5 * vnoise(vec2(p.x * 0.0027 + t * 0.018, p.y * 0.0011 - t * 0.021), 213u);
    let band1 = smoothstep(0.15, 0.85, cur1);
    let band2 = smoothstep(0.25, 0.90, cur2);
    let aurora = vec3(0.13, 1.00, 0.42) * band1 + vec3(0.55, 0.18, 0.95) * band2;
    // Catch factor: puddles mirror the sky, wet earth glows softly, dry dust
    // barely — the SAME moisture story the base look tells, restated in light.
    let wet_catch = 0.10 + 0.18 * moist + 0.55 * pud + 0.15 * snow;
    out.color = vec4(out.color.rgb + aurora * wet_catch, out.color.a);
