...
// ─── THIS LOOK: PULSE NOCTURNE (wet-nocturne variant, ANIMATED) ──────────────
// Same wet-ground structure as wet_nocturne, with a BREATHING bioluminescence:
// the soaked rims around each puddle and the meter-scale mud hollows carry a
// faint teal glow that swells and fades on a ~9 s cycle, phase-drifted by a
// macro noise field so waves of light roll across the wetlands instead of the
// whole world blinking in sync. Glow is added POST-lighting (emission, not
// albedo) and rides the SAME wet masks the base look built — no new geography.
// Masks and octaves are byte-identical to wet_nocturne.
...
fn fragment(
    ...
    // sits in hollows, not only in lakes). `var` at fn scope: the hollows
    // double as the bioluminescence mask in the post-lighting stage below.
    var wet_micro = 0.0;
    let mh_f = footprint_fade(2.5, fw);
    if mh_f > 0.001 {
        let mh = vnoise(p / 2.5, 123u) * 0.7 + vnoise(p / 0.8, 124u) * 0.3;
        wet_micro = smoothstep(0.15, 0.55, mh + (moist - 0.5)) * (1.0 - pud) * (1.0 - snow) * mh_f;
    ...
    // Bioluminescent pulse (ANIMATED): teal emission in the soaked puddle rims
    // and mud hollows, breathing on a ~9 s cycle. The phase is offset by a
    // 200 m noise field, so the glow arrives as slow WAVES rolling across the
    // wetlands — neighbouring pools breathe near-together, distant ones
    // don't. Squared so each swell blooms and lingers dark between breaths.
    let t = globals.time;
    let phase_drift = vnoise(p / 200.0, 218u) * 2.5;
    let breath = pow(0.5 + 0.5 * sin(t * 0.70 + phase_drift), 2.0);
    let glow_mask = rim * 1.0 + wet_micro * 0.6 + pud * 0.12;
    out.color = vec4(
        out.color.rgb + vec3(0.06, 0.50, 0.42) * glow_mask * breath * 0.30,
        out.color.a,
    );
