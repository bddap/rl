...
//! each world's full dynamic state via the SHARED [`crab_world::bot::physics_digest`] layout (the
...
// [`crab_world::physics::PHYSICS_HZ`] (64 Hz) physics steps across [`crate::sim::TICK_HZ`]
...
use crate::lockstep::{Fault, INPUT_DELAY, Lockstep};
use crate::sim::{Input, PlayerId, Pos, TICK_HZ, UNIT, buttons};
...
fn bridge_pose_and_digest(app: &mut App, weights_digest: u64) -> (Pos, u64) {
    ...
    let digest = crab_world::bot::physics_digest::crab_state_digest(q.iter(app.world())) ^ weights_digest;
